freq_dummyvars <- function(df, name_dummy_variables){
  ...
  lookup_table <- df %>%
    ...
    pivot_longer(-lga_face, names_to = "variable", values_to = "values") %>%
    distinct(lga_face, variable) %>%
    group_by(lga_face) %>%
    slice(rep(1:n(), each = n_answer_options))
  
  # dimensions of the look-up table
  lgas_in_lookup <- length(unique(lookup_table$lga_face))
  questions_in_lookup <- length(unique(lookup_table$variable))
  ...
  data_table_srvyr <- df %>%
    ...
    pivot_longer(-c(lga_face, weights_var, ward_face), names_to = "variable", values_to = "value") %>% 
    mutate(answer = ifelse(value == 1, "yes", "no")) %>% 
    as_survey_design(ids = ward_face, weights = weights_var) %>% 
    group_by(lga_face, variable, answer) %>% 
  ...
  final_table <- left_join(lookup_table, data_table_srvyr) %>% 
    replace(is.na(.), 0) %>% 
    group_by(lga_face, variable) %>% 
    mutate(total_participants = sum(subgroup_total)) %>% 
